...
export default async function BeritaAdminPage() {
  const berita = await query<any>(
    "SELECT id, judul as name, slug, created_at, is_published FROM berita ORDER BY created_at DESC"
  );
        ...
        <table className="admin-table">
          ...
          <tbody>
            {berita.length === 0 ? (
              ...
              <tr key={item.id}>
                <td style={{ color: "#c8c2e8", fontWeight: 500, width: "40px" }}>{item.id}</td>
                <td>
                  <div style={{ fontWeight: 600, color: "#f8f8ff", fontSize: "13px" }}>
                    {item.name}
                  </div>
                </td>
                <td className="admin-col-hide-mobile" style={{ color: "#c8c2e8", fontSize: "13px", whiteSpace: "nowrap" }}>
