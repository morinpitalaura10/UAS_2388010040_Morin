...
export default async function LayananAdminPage() {
  const layanan = await query<any>("SELECT id, nama as name, icon, deskripsi, urutan FROM layanan ORDER BY urutan ASC");
          ...
          <tbody>
            {layanan.length === 0 ? (
              ...
              <tr key={item.id}>
                ...
                <td>
                  <div style={{ fontWeight: 700, fontSize: "14px", color: "#f8f8ff" }}>{item.name}</div>
                </td>
                <td className="admin-col-hide-mobile">
                  <div style={{ fontSize: "13px", color: "#c8c2e8" }}>
                    {item.deskripsi}
                  </div>
                </td>
                <td>
                  <code style={{ fontSize: "12px", background: "rgba(255,255,255,0.08)", padding: "3px 8px", borderRadius: "6px", color: "#c8c2e8" }}>
                    {item.icon}
                  </code>
                </td>
                <td>
